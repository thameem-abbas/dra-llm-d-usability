...
KEP-5517 (ResourcePoolStatusRequest), alpha in v1.36, introduces a purpose-built API for querying device pool availability before submitting workloads, using a request/status pattern similar to CertificateSigningRequest. For webhooks that scan ResourceSlices directly to check device availability (like Pattern 1's preflight checking), this API would provide a cleaner alternative to raw slice enumeration. It is currently behind a feature gate, but as it matures it could replace direct ResourceSlice scanning in preflight checkers with a supported, intent-based API designed for exactly this use case.
